...
contract AlligatorOPV2 is IAlligatorOPV2, Ownable, Pausable {
    ...
    {
        endpoint = address(
            new AlligatorProxy{salt: keccak256(abi.encode(proxyOwner, proxyRulesHash))}(
                governor
            )
        );
    ...
    function validate(
        ...
        _validateRules(
            _formatBaseRules(baseRules), sender, authorityLength, proposalId, support, address(0), address(0), 1
        );
